...
export const cellEmpty = function (
...
export const insertMove = function (
  board: TicTacToeBoard,
  position: BoardPosition,
  player: TicTacToeCell
): boolean {
  const [x, y] = position;
...
export const gameState = function (
  ...
  const same = (values: TicTacToeCell[]) =>
    values.every((v) => v === values[0]);
  const winnerByCell = (
    cell: TicTacToeCell
  ): Exclude<TicTacToeResult, "Draw"> =>
  ...
  if (same([board[0][0], board[1][1], board[2][2]]))
    return winnerByCell(board[0][0]);
  if (same([board[0][2], board[1][1], board[2][0]]))
    return winnerByCell(board[0][2]);
